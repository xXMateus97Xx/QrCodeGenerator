...
public static class QrSegmentAdvanced
{
    ...
    private const String PACKED_QR_KANJI_TO_UNICODE =
    ...
    static QrSegmentAdvanced()
    {
        ...
        var bytes = Convert.FromBase64String(PACKED_QR_KANJI_TO_UNICODE);
    ...
    private static List<QrSegment> MakeSegmentsOptimally(ReadOnlySpan<int> codePoints, int version)
    {
    ...
    private static List<QrSegment> SplitIntoSegments(ReadOnlySpan<int> codePoints, Mode[] charModes)
    {
        if (codePoints.Length == 0)
            throw new ArgumentException();
